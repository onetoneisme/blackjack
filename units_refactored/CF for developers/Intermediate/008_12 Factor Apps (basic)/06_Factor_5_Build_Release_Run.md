...
1. The **build stage** is a transformation, which converts a code repo into an executable bundle known as a build. Using a version of the code at a commit specified by the deployment process, the build stage fetches and vendors dependencies and also compiles binaries and assets.
...
**The twelve-factor app methodology uses strict separation of the build, release, and run stages.** For example, it is impossible to make changes to the code at runtime, since there is no way to propagate those changes back to the build stage.

The **release** and **run** stages in Cloud Foundry look as follows:
```
Creating container
